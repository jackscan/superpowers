...
```bash
# The script backgrounds the server itself — run it normally.
scripts/start-server.sh --project-dir /path/to/project --open
```

On Windows, the script auto-detects and switches to foreground mode (which blocks the tool call). Run the command in the background via your platform's background execution mechanism so the server survives across conversation turns, then read `$STATE_DIR/server-info` on the next turn to get the URL and port.

**If your environment reaps detached processes:** use `--foreground` and launch the command with your platform's background execution mechanism so the server survives across turns.
